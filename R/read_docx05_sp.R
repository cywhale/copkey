...
find_subfigx <- function(xstr, subfig, idx, print_info=TRUE) {
  ...
  if (grepl("^((F|f)emale|(M|m)ale)", xsubfx)) {
    return(regexpr(tolower(xsubfx), xstr))
  }
...
italics_exclude_wordx <- function (x, skip_word, subto, append="") {
  xs1 <- substr(x, 1, subto)
  if (grepl(skip_word, xs1)) {
            ...
            if (i>=tstL | (!is.na(x) & (wa>0 | (xsp1==gen_name & imgj>0 & imgj>=length(subfig))))) { #trimx(sp2namex(x)) != xsp2)) {
              ...
              if (i>=tstL | (!is.na(x) & (spt != spname | (spt == spname & imgj>0 & imgj>=length(subfig))))) {
                ...
                if (length(fig_caption)==0) {
                  ...
                  cap_cite[is.na(cap), cap:=fig_caption[!is.na(fig_caption)][1]] 
                ...
                if (length(subfig)==0 & length(fig_caption)==0) { #only one fig in this row and not read any fig_caption yet
                  ...
                  flag_getcap <- FALSE
                           ...
                           (imgj<=k & ((any(grepl("(F|f)emale",subfig)) & grepl("(F|f)emale", x)) |
                                       (any(grepl("Male", subfig)) & grepl("Male", x))))) {
                  flag_getcap <- TRUE
                ...
                if (flag_getcap) {    
                  ...
                  if (length(subfig)>0) {
                    ...
                    if (imgj<=k & (any(grepl("(F|f)emale",subfig)) & grepl("(F|f)emale", x)) &
                          (any(grepl("Male", subfig)) & grepl("Male", x))) {
                        k <- 2 # one description (x) contains two subfigs
                    } else if (xc[1]>0) {
                      if (imgj==0 & length(xc)>1 & xc[1]>0) {
                        k <- length(xc)
                  ...
                  if (length(xc)>1 & k>1) {
                    ...
                    if (!grepl(iprex, xseg[1])) { xseg <- xseg[-1] }
