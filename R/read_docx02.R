...
IndexVers<- 2L ## Shih's document version
EnableBlkFig <- TRUE ## Enable Block figure within key-column (main-column)
...
##### Note manually edit doc/Table of key... -> Fig_key_tbl.csv for first column Fig. 8,9 to two rows
  ...
  dc0 <- read_docx("doc/New_Version_Key2.docx")
  ...
  if (!st_conti_flag) {
    ...
    if (attributes(wl)$match.length>0) {
      ...
      if (IndexVers==1L) {
        ...
        pret <- paste0(pret,'</div><div><p class=leader><span class=',dQuote('keycol'),'>',
                       '<mark id=', dQuote(paste0('key_', keyx)), '>', keyx, '</mark> ')
      }
...
#  }
   ...
   if (EnableBlkFig & !any(is.na(figx)) & (pret_case!=0L | fig_conti_flag |## WaitFlush will hold-on fig-flush
