...
var gblfx = function() {
  ...
  var genx = "";
  if (pars.hasOwnProperty('taxon')) { //plugin.settings.hashGen  // it means first-link copkey by ?taxon=xxx
    genx = pars.taxon; // support another keyword
  } else if (document.location.hash.indexOf('#' + plugin.settings.hashGen) != -1) { //
    genx = document.location.hash.replace('#' + plugin.settings.hashGen, '');
  }

  if (genx !== "") {
    window.history.replaceState({}, document.title, clean_uri + "#" + plugin.settings.hashGen  + genx);
  }
        ...
        var hash_scroller = function(targ, hash_type, hash_id) {
            ...
            if (idxp==-1) {
              alert("Key, fig or taxon not found: " + hash_id);
        ...
        if(plugin.settings.pages >= 1) {
          ...
          displayNav();
          page = 1;
          if (pagex>0 || (figx===0 && keyx===0 && genx==="")) { //document.location.hash.indexOf('#'+plugin.settings.hashPage) != -1 || (!figx && !keyx)
            if (pagex) { page = pagex; }
            ...
            if (document.location.hash.indexOf('#'+plugin.settings.hashKey) != -1) {
              ...
              hash_scroller($('#' + plugin.settings.hashGen + genx),  plugin.settings.hashGen, genx);
            }
          }
        }
        //}
